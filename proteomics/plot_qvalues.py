...
def estimateQvalues(combined,pi0=1.0,prefix="random",order=False,Ties=False,fdr=0.01):
    ...
    return sorted(qvaluesEmp),sorted(qvaluesEst),ndecoys_fdr,ntargets_fdr


def estimateFDR_hidden_decoys(target,pi0=1.0,prefix="entrapment",order=False,Ties=False,fdr=0.01):
    ...
    for hit in target:
        ...
        if(ntarget > 0):
            qemp = (float(nrandom) * 2)  / float(ntarget + nrandom)
            if(Ties):
                qest = float(suma) / float(ntarget + nrandom)
            else:
                qest = float(suma) / float(ntarget)
    ...
    print "Estimating qvalues with " + str(ntarget) + " targets and " + str(nrandom) + " decoys"
      
    if(ntarget > 0 and nrandom > 0):    
        factor = float( pi0 * (float(ntarget) / float(nrandom)) )
    else:
        factor = 1.0
...
def usage():
    ...
    print "Usage : plot_qvalues.py <metaFile.txt>  [d, --prefix <prefix>]  [e, --hidden <pattern>] [l, --label <text>] [f, --fdr <number>] [i, --pi0] [v, --verbose] [h, --help]"
...
def main(argv):
    if( len(argv) < 1):
        ...
        pi0 = False
        try:
            opts, args = getopt.getopt(sys.argv[2:], "e:d:vhl:f:i:", ["hidden=", "prefix=", "verbose", "help", "label=", "fdr=", "pi0="])
        ...
        for x in xrange(len(hits)):
            ...
            ntargets = 0
            if(hidden_decoys):
                qvaluesEmps[x],qvaluesEsts[x],ndecoys,ntargets = estimateFDR_hidden_decoys(hits[x],pi0s[x],hidden_pattern,False,False,fdr)
            else:
                qvaluesEmps[x],qvaluesEsts[x],ndecoys,ntargets = estimateQvalues(hits[x],pi0s[x],decoy_pattern,False,False,fdr)
            if(pi0):
                print "pi0 file " + str(x+1) + " :" + str(pi0s[x])
            print "elements with estimated qvalue below " + str(fdr) + " in file " + str(x+1) + " :"  + str(ntargets) 
            print "elements with empirical qvalue below " + str(fdr) + " in file " + str(x+1) + " :"  + str(len([q for q in qvaluesEmps[x] if q <= fdr])) 
            print "false positive elements with estimated qvalue below " + str(fdr) + " in file " + str(x+1) + " :"  + str(ndecoys) 
            #print "false positive elements with empirical qvalue below " + str(fdr) + " in file " + str(x+1) + " :"  + str(len([q for q in qvaluesEmps[x] if q <= fdr])) 
